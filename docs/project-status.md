...
- Current story: STO-001 — Initialize SQLite and migrations
- Overall state: capture domain complete; storage foundation ready
...
ports. There are no production adapters, Tauri code, migrations, or persisted
models yet.
...
entry points, domain invariants, and application orchestration with fakes.
...
vulnerabilities. STO-001 is active.

The active story changes only the storage adapter and introduces real SQLite
integration tests. Domain and application packages remain independent of it.

## Next steps

1. Select and constrain the SQLite runtime dependency.
2. Implement configurable database opening and transactional migrations.
3. Verify WAL, foreign keys, idempotency, and rollback behavior.
4. Report migration recovery guidance without destructive automation.
